...
import com.artur114.bananalib.mc.client.audio.SoundTile;
...
public class TileSoundsService implements IService {
    private final Map<BlockPos, TileSoundEntry<?>> soundTileEntryMap = new HashMap<>();
    ...
    public void stopTileSound(BlockPos pos) {
        TileSoundEntry<?> sound = this.soundTileEntryMap.get(pos);

        if (sound != null) {
            sound.remove(); this.soundTileEntryMap.remove(pos);
        }
    }
