...
class Matrix2FMTest extends TestBase {
    ...
    void "inverse determinant test"() {
        multiTest {
            def m = randomMatrix()
            assert equalsEps(m.copy().invert().determinant(), (float) (1.0F / m.determinant()))
        }
    }
    ...
    void "invert matrix restore point test"() {
        multiTest {
            withRandMatrix { Matrix2FM m, tr, rot, scale ->
                IVec2D point = randomPoint()
    ...
    void "determinant test"() {
        ...
        multiTest {
            withRandMatrix {Matrix2FM m, tr, rot, IVec2D scale ->
    ...
    def randomMatrix() {
        Matrix2FM m = new Matrix2FM()
        if (rand.nextDouble() < 0.5) {
            m.translate(randInt(40), randInt(40))
            m.rotate(this.rand.nextFloat() * 360.0F as float)
    ...
    def withRandMatrix(Closure<Void> cl) {
        ...
        cl.call(new Matrix2FM().translate(tr.x(), tr.y()).rotate(rot).scale(scale.x(), scale.y()), tr, rot, scale)
    }
