...
class Matrix3DMTest extends TestBase {
    @Override
    int defMultiCount() {
        return 1000
    }
    ...
    void "inverse transform test"() {
        multiTest {
            def m = randomMatrix
            def v = randVec3d
            assert m.copy().invert().transform(m.transform(v)).equalsEps(v)
        }
    }
    ...
    void "matrix2 rotate test"() {
        multiTest(80) {
            ...
            assert m2.copy().rotate(rot).transform(vec).equalsEps(m3.copy().rotateZ(rot).transform(vec))
        }
    }
    ...
    void "vec and matrix rotate test"() {
        multiTest {
            ...
            assert m.copy().rotateX(rot).transform(vec).equalsEps(vec.rotateX(rot))
            assert m.copy().rotateY(rot).transform(vec).equalsEps(vec.rotateY(rot))
            assert m.copy().rotateZ(rot).transform(vec).equalsEps(vec.rotateZ(rot))
        }
    }
    ...
    void "invert matrix test"() {
        multiTest {
            ...
            assert m.copy().mul(m.copy().invert()).equalsEps(identityMatrix)
        }
    }
