...
class Matrix3FTest extends TestBase {
    ...
    void "inverse transform test"() {
        multiTest {
            def m = randomMatrix
            def v = randVec3d
            assert m.invert().transform(m.transform(v)).equalsEps(v, BananaMath.FLOAT_EQUALS_EPS)
        }
    }
    ...
    void "identity transform test"() {
        multiTest {
            ...
            assert mat.mul(identityMatrix).equalsEps(mat, BananaMath.FLOAT_EQUALS_EPS)
        }
    }
    ...
    void "inverse determinant test"() {
        multiTest {
            def m = randomMatrix
            assert equalsEps(m.invert().determinant(), 1.0F / m.determinant(), BananaMath.FLOAT_EQUALS_EPS)
        }
    }
    ...
    void "vec and matrix rotate test"() {
        multiTest {
            ...
            assert m.rotateX(rot).transform(vec).equalsEps(vec.rotateX(rot), BananaMath.FLOAT_EQUALS_EPS)
            assert m.rotateY(rot).transform(vec).equalsEps(vec.rotateY(rot), BananaMath.FLOAT_EQUALS_EPS)
            assert m.rotateZ(rot).transform(vec).equalsEps(vec.rotateZ(rot), BananaMath.FLOAT_EQUALS_EPS)
        }
    }
